...
struct client_hub_no_ioc :private traits,hub_base<client_hub_no_ioc<traits>, web_socket<typename traits::socket_type, client_hub_no_ioc<traits>>> {
	...
	void remove_socket(web_socket_type* s) {
		std::lock_guard<std::mutex> lock(m_sockets_mut);
		const auto it = std::find_if(m_sockets.begin(), m_sockets.end(), [&](const auto& a) {return a.get() == s; });
		//[[assert: it!=m_sockets.end()]];
		std::swap(*it, m_sockets.back());
		m_sockets.pop_back();
	}
...
private:
	...
	task<void> m_on_resolve(endpt&& end_points, std::string_view port, std::string_view uri,std::string_view target,std::promise<web_socket_type*> promise,rng&& extra_headers,string_t&& body,std::string full_url) {
		try{
			...
			for(auto&& r:end_points){
				sockland.open(r.endpoint().protocol());
				sockland.set_option(boost::asio::ip::tcp::no_delay(true));
				ec = co_await sockland.async_connect(r.endpoint(),use_task_return_ec);
				if (!ec) 
					break;
				sockland.close();				
			}
			...
			auto socky = std::make_unique<web_socket_type>(this->create_socket(std::move(sockland)),this);
			...
			std::cout << uri << ' ' << target << std::endl;
			co_await socky->socket().async_handshake_ex(std::string(uri), std::string(target), [&](auto&) {}, use_task);
			
			auto ptr = socky.get();
			promise.set_value(ptr);
			{
				std::lock_guard<std::mutex> lock(m_sockets_mut);
				m_sockets.push_back(std::move(socky));
			}
			this->on_connect(*ptr);

			//ptr->do_read();
			ptr->start_async_read();
		}catch(...) {
			std::cout << "r;-=;" << std::endl;
	...
	std::mutex m_sockets_mut;
	std::vector<std::unique_ptr<web_socket_type>> m_sockets{};//;-;
